...
(async () => {
  ...
  if (data.length > 0) {
    ...
    data.forEach(blog => {
      const date = new Date(blog.
        createdAt).toDateString();
      const html = `<div class="blogcard">
        <img src="${blog.img1}"/>
        <div class="alldes">
        <p class="date">${date}</p>
        <h3><a class="topic">${blog.title}</a></h3>
      ...
      <div class="comment">
       ...
       <p class="comments">${blog.comments.length}</p>
       ...
       data-id='${blog._id}'
        ...
        <a data-id='${blog._id}' class="delete"
        ><svg
        class="delete"
        data-id='${blog._id}'
         xmlns="http://www.w3.org/2000/svg"
...
// Deleting blogs
blogs.addEventListener('click', async function (event) {
